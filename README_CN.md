...
这是一个基于 AD5941 和 ESP32-C3 的差分脉冲伏安法 DPV 电化学检测项目，目标是做一个低成本、小型化、可以用于生化实验室测试的电化学检测板。板子通过 AD5941 完成电化学激励、电流采集和模数转换，通过 ESP32-C3 运行 Arduino 驱动程序，并通过串口把 DPV 数据发送到电脑上位机实时显示和保存。

硬件支持 3 个工作电极 WE1、WE2、WE3，通过 ADG704 模拟开关分时切换。当前已经分别验证过 WE1、WE2、WE3 单通道工作正常。当前推荐使用串口上位机模式。之前尝试过 WiFi 网页显示和 TF 卡存储，但测试发现 TF 卡模块接入同一 SPI 总线后会干扰 AD5941 通信，所以当前稳定版本暂时关闭 TF 卡和 WiFi，优先保证实验室测试可靠。
...
本项目已经用电阻负载做过基本校准。使用 100 k ohm 电阻时，0 到 200 mV 扫描下测得电流与理论值基本一致，说明电压输出、电流采集和串口上传流程是通的。
...
目前需要特别注意：① TF 卡功能不建议使用，因为测试中发现 TF 卡模块接入后会影响 AD5941 的 SPI 通信，尤其会导致 AD5941 ID 读取异常；② WiFi 网页功能目前也不作为推荐版本使用，寄到实验室的测试版本优先使用串口上位机；③ 外部 RTIA 相关电阻目前只建议用于硬件诊断，不建议作为正式 DPV 的 RTIA 路径；④ DPV 结果对电极状态、溶液浓度、气泡、夹子接触、静置时间、脉冲幅度和采样时刻都很敏感，结果波动大时应先确认实验条件，再调整驱动参数。

6. 后续计划

后续计划主要包括：① 用不同浓度铁氰化钾溶液做标定曲线，比较自制板和商业电化学工作站的峰电流、峰电位和重复性；② 优化 DPV 采样时刻、平均方式和抗噪声处理，让曲线更平滑；③ 做 WE1、WE2、WE3 的重复性测试，确认多工作电极切换是否会带来系统偏差；④ 如果后续要恢复 TF 卡或 WiFi，需要优先修改硬件连接或更换不会干扰 SPI 总线的模块。

7. 仓库内容

本仓库主要包含 Arduino 驱动代码、AD5941/AD5940 相关支持文件、ESP32-C3 和 AD5941 的引脚配置、DPV 参数说明、板子功能说明、原理图文件，以及铁氰化钾测试结果图片和商业工作站对照图片。
